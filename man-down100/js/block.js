 ...
 * 各种障碍物块的定义
 */
let BlockBase = function (x, y, img, cxt, panelInfo) {
	// 障碍物基类
	this.x = x;
	this.y = y;
	this.img = img;
...
NormalBlock.prototype.initSprite = function () {
	//初始化普通障碍物
	let sprite = new WF.sprite.Sprite(this.img, this.cxt, 1, { x: this.x, y: this.y, yspeed: this.yspeed });
...
let MissBlock = function (x, y, img, cxt, panelInfo) {//消失障碍物
	BlockBase.apply(this, arguments);
...
let LeftBlock = function (x, y, img, cxt, panelInfo) {
	//左滑障碍物
	BlockBase.apply(this, arguments);
	this.xforce = -4;
}
...
LeftBlock.prototype.initSprite = function () {
	//初始化左滑障碍物
	let sprite = new WF.sprite.Sprite(this.img, this.cxt, 5, { x: this.x, y: this.y, yspeed: this.yspeed });
...
let RightBlock = function (x, y, img, cxt, panelInfo) {//右滑障碍物
	BlockBase.apply(this, arguments);
	this.xforce = 4;
}
...
let FlipBlock = function (x, y, img, cxt, panelInfo) {
	//弹跳障碍物
	BlockBase.apply(this, arguments);
	this.flipcount = 5;
	this.isStand = false;
...
FlipBlock.prototype.changeDir = function (dir) {
	//弹跳障碍物改变方向
	let o_size = this.sprite.size();
	this.sprite.change(dir);
	let n_size = this.sprite.size();
	let y = (o_size.h - n_size.h) + o_size.y;
...
var BlockFactory = {
	...
	create: function () {//创建
		...
		let rnd_x = Math.floor(Math.random() * 224);

		let x = rnd_x;
		let y = 5000;       // 此处修改障碍物生成的最低位置
