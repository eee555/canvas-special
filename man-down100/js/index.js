...
let Main = {
	...
	initBlock: function (imgs) {
		...
		for (i = 0; i < 46; i++) { 
			let block = new NormalBlock(Math.floor(Math.random() * 224), 400 + 100 * i, imgs[1], Main.cxt, Main.gameInfo);
			block.init();
			Main.blocks.push(block);
		}
		// let block = new NormalBlock(120, 400, imgs[1], Main.cxt, Main.gameInfo);
		// block.init();
		// Main.blocks.push(block);
	},
	initEvent: function () {
	...
	process: function () {
		//游戏运行
		let tq = new WF.time.TimeProcess();
